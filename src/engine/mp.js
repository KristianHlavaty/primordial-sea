...
function makeRenderPlayer(engine, pd) {
  ...
  return {
    ...
    hp: pd.hp, maxHp: pd.mhp, level: pd.lv, shield: pd.sh, biteAnim: pd.b, kills: pd.k || 0, deadT: pd.d || 0,
  };
}
...
function applySnapshot(engine, s) {
  ...
  for (const pd of s.players) {
    if (pd.c === mp.self) {
      ...
      pl.gx = pd.x; pl.gy = pd.y; pl.ga = pd.a; pl.hp = pd.hp; pl.maxHp = pd.mhp; pl.level = pd.lv; pl.shield = pd.sh;
    ...
    else if (rp.species !== pd.s) { rp.species = pd.s; rp.plan = SPECIES[pd.s].plan; rp.radius = SPECIES[pd.s].stats.radius; }
    rp.gx = pd.x; rp.gy = pd.y; rp.ga = pd.a; rp.hp = pd.hp; rp.maxHp = pd.mhp; rp.level = pd.lv; rp.shield = pd.sh;
...
export function mpClientUpdate(engine, dt) {
  ...
  const decay = e => { e.biteAnim = Math.max(0, (e.biteAnim || 0) - dt * 3); e.mouth = e.biteAnim; e.hurt = Math.max(0, (e.hurt || 0) - dt * 3); };

/* Root component: the game canvas plus whichever UI layer the game state
   calls for (HUD, toasts, tree wiki, evolve/pause/death/start screens). */
...
export function App() {
  ...
  uiRef.current = { phase, treeOpen, openTree, closeTree };

  return html`
    <${Fragment}>
      <canvas id="game" ref=${canvasRef}/>

      ${phase === 'play' && hud && !hud.dead && html`<${Hud} hud=${hud} engine=${engine} onOpenTree=${openTree}/>`}
      ...
      ${phase === 'play' && hud && hud.paused && !hud.pendingEvolve && !hud.dead && !treeOpen && html`<${PauseOverlay} onResume=${() => engine.togglePause()}/>`}

      ${phase === 'play' && hud && hud.dead && html`<${GameOverScreen} hud=${hud} onRestart=${begin}/>`}

      ${phase === 'start' && html`<${StartScreen} onBegin=${begin}/>`}
    <//>`;
}

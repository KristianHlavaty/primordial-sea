...
function roomList() {
  return [...rooms.values()].map(r => ({
    ...
    fantasy: r.fantasy, evolution: r.evolution, count: r.members.length, maxPlayers: r.maxPlayers, started: r.started,
  }));
}

function roomDetail(r) {
  return {
    id: r.id, name: r.name, map: r.map, mapName: r.mapName, stage: r.stage, tier: r.tier, era: r.era, fantasy: r.fantasy, evolution: r.evolution,
  ...
  switch (m && m.t) {
    ...
    case 'createRoom': {
      ...
      const r = {
        ...
        fantasy: m.fantasy === true, evolution: m.evolution !== false, started: false, members: [conn.id], species: {},
      };
